...
{ 
    char localhost[30] = "127.0.0.1";
    char idList[10][20]; //사용자 아이디는 10개까지 저장 가능
    char temp[20];
    int iDs; // 디스크립터 저장
    ...
    // 서버 포트(포트 문을 열어준다.)
    stAddr.sin_port = htons(atoi(argv[1])); // 포트 3000번 연다.
    ...
    while(1)
    {
        printf("현재 접속자 수 %d\t", iCNum); // 접속자 수 표시
        printf("연결된 포트 %d\n", atoi(argv[1]));
